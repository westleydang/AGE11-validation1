...
# testing roi stuff
  knew.molten2 %>%
      ...
      stat_summary(fun.data = "mean_se", geom="errorbar", aes(width=0.3)) + 
      facet_wrap(~CHANNEL, scales="free") +
      ggtitle(paste(c("Whole brain, weighted, density ROIAreapx"),bright,"cells")) +
      theme_minimal() 
      ...
      filter(BRIGHTNESS == 'total') %>%
      group_by(ID, GROUP, EXPT, CHANNEL, variable) %>%
      summarize(weighted = weighted.mean(value, ROIAreapx_PopMean)) %>%
      ggplot() + aes(GROUP, weighted, color=EXPT) + 
      stat_summary(fun.y = "mean", geom="point") + 
      stat_summary(fun.data = "mean_se", geom="errorbar", aes(width=0.3)) + 
      facet_wrap(~CHANNEL, scales="free") +
      ggtitle(paste(c("Whole brain, weighted, density ROIAreapx_PopMean"),bright,"cells")) +
      theme_minimal() 
      ...
      filter(BRIGHTNESS == 'total') %>%
      group_by(ID, GROUP, EXPT, CHANNEL, variable) %>%
      summarize(weighted = weighted.mean(value, Sample_roiAreaMM2)) %>%
      ggplot() + aes(GROUP, weighted, color=EXPT) + 
      stat_summary(fun.y = "mean", geom="point") + 
      stat_summary(fun.data = "mean_se", geom="errorbar", aes(width=0.3)) + 
      facet_wrap(~CHANNEL, scales="free") +
      ggtitle(paste(c("Whole brain, weighted, density Sample_roiAreaMM2"),bright,"cells")) +
      theme_minimal() 
      ...
      stat_summary(fun.data = "mean_se", geom="errorbar", aes(width=0.3)) + 
      facet_wrap(~CHANNEL, scales="free") +
      ggtitle(paste(c("Whole brain, weighted, density Pop_roiAreaMM2"),bright,"cells")) +
      theme_minimal() 
      ...
      filter(BRIGHTNESS == 'total') %>%
      group_by(ID, GROUP, EXPT, CHANNEL, variable) %>%
      summarize(weighted = weighted.mean(value, areaNormFactor)) %>%
      ggplot() + aes(GROUP, weighted, color=EXPT) + 
      stat_summary(fun.y = "mean", geom="point") + 
      stat_summary(fun.data = "mean_se", geom="errorbar", aes(width=0.3)) + 
      facet_wrap(~CHANNEL, scales="free") +
      ggtitle(paste(c("Whole brain, weighted, density areaNormFactor"),bright,"cells")) +
      theme_minimal() 
...
# if we did a 4-way ANOVA
sys.stats = knew.molten3 %>%
  group_by(CHANNEL) %>%
  do(tidy(aov(data=.,value ~ System * EXPT * VALENCE * CONTEXT)))

sys.stats = knew.molten3 %>%
  group_by(CHANNEL, System) %>%
  do(tidy(aov(data=.,value ~ EXPT * VALENCE * CONTEXT)))
summary(sys.stats$p.value)
