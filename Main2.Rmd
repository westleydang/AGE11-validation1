---
title: "R Notebook for AGE 11 validation 1"
output:
...
# make GRAIN, merge BATCH details
batch_details = read.csv("batch_details.csv")
kk.long$GRAIN = paste(kk.long$GROUP, kk.long$EXPT)
colnames(batch_details) = c("GRAIN", "BATCH")
kk.long = merge(kk.long, batch_details, by="GRAIN")
...
kkl.collapseROI = kk.long %>%
  group_by(CHANNEL, BATCH, GRAIN, GROUP, VALENCE, CONTEXT, EXPT) %>%
...
ggplot(kkl.collapseROI) + aes(BATCH, mean_density) + facet_wrap(~CHANNEL, scales="free") +
  stat_summary(fun.y = mean, geom="point") +
  stat_summary(fun.data = mean_se, geom="linerange") +
  ggtitle("--> comparing batches A, B, C")

```

## 2.1 Interpretations of these graphs:
2.1.1. DAPI counts are fairly similar between young/age groups, which suggests to me that the counts are somewhat normal, except between AA vs AB in the young animals, which I know are different batches. 
2.1.2. Old animals consistently have higher densities, what are possible reasons? 
- Is it because the staining for these animals were done at a later time and were probably better? 
2.1.3. How do we know for sure that the young vs. old aren't reflectve of technical artifacts?
...
2.1.4. From these graphs it looks like we might not be able to make any real inferences between the old/young animal sets, but what we can do is compare the groups within animal sets, and how those differences compare to each other.
...
# III. Graphing means for each sub-ROI

**Purpose.** This data is only accurate at the sub-ROI level, because the aggregated data in the previous graphs are not weighted averages. We want to see if any brain areas present any bigger differences than others. 

**Method.** What are the best comparisons that we want to make between each sub-ROI?
1. Young vs. old. 
2. AA vs. AB for shock vs 

```{r, echo=T,fig.width=12, fig.height=16, message=FALSE, warning=FALSE}
...
kkl.collapseID = kk.long %>% 
  group_by(CHANNEL, GRAIN, BATCH, CONTEXT, GROUP, EXPT, SUBROI) %>%
    summarize(sem = std.error(DENSITY), mean_density = mean(DENSITY), N = length(DENSITY),
            ymin = mean_density-sem, ymax = mean_density+sem)
kkl.collapseID
...
```{r,echo=T,fig.width=4,fig.height=2}

ggplot(kk) + aes(x=reorder(SUBROI, AREAmm2), AREAmm2) + 
  stat_summary(fun.y = mean, geom="point") + 
...
# Densities per ROI, within-BATCH comparison (AA vs AB)
- I want to compare only young animals, and CAA/CAB and also SAA/SAB
- Best way to do this would be to plot out (subroi, mean_density)
- Then facet by BATCH, and also fill by CONTEXT

```{r, echo=T}

# check the AA/AB for young animals
ggplot(kkl.collapseID %>% filter(EXPT=='_YOUNG') %>% filter(GROUP=='SAA' | GROUP=='SAB')) + aes(SUBROI, mean_density, color = CONTEXT) +
  stat_summary(fun.y = mean, geom = "point") +
  stat_summary(fun.data = mean_se, size=0.5, geom="linerange") +
  coord_flip() + 
