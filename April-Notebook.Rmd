...
# Find weighted average of brain areas
```{r}
kk.longwm = kk.long

kk.longwm$DENSITY = kk.longwm$DENSITY/kk.longwm$AREAmm2

kkl.collapseROI.wm = kk.longwm %>%
  group_by(CHANNEL, GROUP, VALENCE, CONTEXT, EXPT, ID) %>%
  summarize(sem = std.error(DENSITY), mean_density = mean(DENSITY), N = length(DENSITY),
            ymin = mean_density-sem, ymax = mean_density+sem)
...
ggplot(kkl.collapseROI.wm) + aes(GROUP, mean_density, fill=EXPT, color = EXPT) + 
  facet_wrap(~CHANNEL, scales="free") +
  stat_summary(fun.y = mean, geom="point") +
  stat_summary(fun.data = mean_se, geom="linerange") +
  ggtitle("Comparing densities per channel ~ group/age, where n = # animals ")

ggplot(kkl.collapseROI.wm) + aes(VALENCE, mean_density, fill=EXPT, color = EXPT) + facet_wrap(~CHANNEL, scales="free") +
  stat_summary(fun.y = mean, geom="point") +
  stat_summary(fun.data = mean_se, geom="linerange") +
  ggtitle("--> comparing context vs. shock")

ggplot(kkl.collapseROI.wm) + aes(CONTEXT, mean_density, fill=EXPT, color = EXPT) + facet_wrap(~CHANNEL, scales="free") +
  stat_summary(fun.y = mean, geom="point") +
  stat_summary(fun.data = mean_se, geom="linerange") +
  ggtitle("--> comparing A/A vs. A/B")
...
```{r, echo=F, fig.width = 8, fig.height = 4, message=FALSE, warning=FALSE}
