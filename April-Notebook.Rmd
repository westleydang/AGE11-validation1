...
## 1.5. Details about this dataset
...
```{r, echo=F, fig.width = 4, fig.height = 2, message=FALSE, warning=FALSE}

library(plyr)
shock.induction = ddply(kk.long, .(CHANNEL, BATCH), transform, norm = scale(DENSITY))
...
ggplot(shock.induction %>% filter(CHANNEL=="DAPI")) + 
  aes(reorder(SUBROI, DIFFNORM), DIFFNORM, color=EXPT) + 
  stat_summary(fun.y = mean, geom="point") +
  stat_summary(fun.data = mean_se, geom="linerange") +
  geom_hline(yintercept = 0) + 
  ...
  # coord_flip() +
  ggtitle("Relative shock induction in DAPI") + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
ggplot(shock.induction %>% filter(CHANNEL=="GFAP")) + 
  aes(reorder(SUBROI, DIFFNORM), DIFFNORM, color=EXPT) + 
  stat_summary(fun.y = mean, geom="point") +
  stat_summary(fun.data = mean_se, geom="linerange") +
  geom_hline(yintercept = 0) + 
  facet_wrap(~CONTEXT2, ncol=1) +
  # coord_flip() +
  ggtitle("Relative shock induction in GFAP") + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
ggplot(shock.induction %>% filter(CHANNEL=="OL")) + 
  ...
  facet_wrap(~CONTEXT2, ncol=1) +
  # coord_flip() +
  ggtitle("Relative shock induction in OL") + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
ggplot(shock.induction %>% filter(CHANNEL=="NormOL")) + 
  aes(reorder(SUBROI, DIFFNORM), DIFFNORM, color = EXPT) + 
  stat_summary(fun.y = mean, geom="point") +
  stat_summary(fun.data = mean_se, geom="linerange") +
  geom_hline(yintercept = 0) + 
  facet_wrap(~CONTEXT2, ncol=1) +
  # coord_flip() +
  ggtitle("Relative shock induction in NormOL") + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1))



aa.induction = ddply(kk.long, .(CHANNEL, SUBROI, BATCH),
                     transform, norm = scale(DENSITY))
aa.induction = dcast(aa.induction, CHANNEL + SUBROI + EXPT + VALENCE ~ CONTEXT,
                     value.var = "norm", fun.aggregate = mean)
aa.induction$DIFFNORM = aa.induction$AA - aa.induction$AB


ggplot(aa.induction %>% filter(CHANNEL == 'DAPI')) +
  aes(reorder(SUBROI, DIFFNORM), DIFFNORM, color=EXPT) +
  stat_summary(fun.y = mean, geom = "point") +
  stat_summary(fun.data = mean_se, geom='linerange') +
  geom_hline(yintercept = 0) +
  ggtitle("Relative AA induction in DAPI") +
  theme(axis.text.x = element_text(angle = 90, hjust=1)) +
  facet_wrap(~VALENCE, ncol=1) 
  # coord_flip() +

ggplot(aa.induction %>% filter(CHANNEL == 'H2BGFP')) +
  aes(reorder(SUBROI, DIFFNORM), DIFFNORM, color=EXPT) +
  stat_summary(fun.y = mean, geom = "point") +
  stat_summary(fun.data = mean_se, geom='linerange') +
  geom_hline(yintercept = 0) +
  ggtitle("Relative AA induction in H2BGFP") +
  theme(axis.text.x = element_text(angle = 90, hjust=1))  +
  facet_wrap(~VALENCE, ncol=1) 
  # coord_flip() +

ggplot(aa.induction %>% filter(CHANNEL == 'GFAP')) +
  aes(reorder(SUBROI, DIFFNORM), DIFFNORM, color=EXPT) +
  stat_summary(fun.y = mean, geom = "point") +
  stat_summary(fun.data = mean_se, geom='linerange') +
  geom_hline(yintercept = 0) +
  ggtitle("Relative AA induction in GFAP") +
  theme(axis.text.x = element_text(angle = 90, hjust=1))  +
  facet_wrap(~VALENCE, ncol=1) 
  # coord_flip() +

ggplot(aa.induction %>% filter(CHANNEL == 'ArcIHC')) +
  aes(reorder(SUBROI, DIFFNORM), DIFFNORM, color=EXPT) +
  stat_summary(fun.y = mean, geom = "point") +
  stat_summary(fun.data = mean_se, geom='linerange') +
  geom_hline(yintercept = 0) +
  ggtitle("Relative AA induction in ArcIHC") +
  theme(axis.text.x = element_text(angle = 90, hjust=1)) +
  facet_wrap(~VALENCE, ncol=1) 
  # coord_flip() +

ggplot(aa.induction %>% filter(CHANNEL == 'OL')) +
  aes(reorder(SUBROI, DIFFNORM), DIFFNORM, color=EXPT) +
  stat_summary(fun.y = mean, geom = "point") +
  stat_summary(fun.data = mean_se, geom='linerange') +
  geom_hline(yintercept = 0) +
  ggtitle("Relative AA induction in OL") +
  theme(axis.text.x = element_text(angle = 90, hjust=1))  +
  facet_wrap(~VALENCE, ncol=1) 
  # coord_flip() +


ggplot(aa.induction %>% filter(CHANNEL == 'NormOL')) +
  aes(reorder(SUBROI, DIFFNORM), DIFFNORM, color=EXPT) +
  stat_summary(fun.y = mean, geom = "point") +
  stat_summary(fun.data = mean_se, geom='linerange') +
  geom_hline(yintercept = 0) +
  ggtitle("Relative AA induction in NormOL") +
  theme(axis.text.x = element_text(angle = 90, hjust=1)) +
  facet_wrap(~VALENCE, ncol=1) 
  # coord_flip() +
...
## 3.1 Cross reference where H2BGFP and ArcIHC inductions are both positive
...
# check the straight up intensity levels

```{r, fig.width = 16, fig.height = 4}
rx = theme(axis.text.x = element_text(angle = 90, hjust = 1))

# intensity.list = c("Ch4.Mean.", "Ch3.Mean.")
# 
# kk$ID = as.factor(kk$ID)
# 
# for (x in intensity.list) {
#   id = "ID"
#   print(
#     ggplot(raw) + aes_string(reorder(id, x), x) + geom_violin() +rx + facet_wrap(~Z.Level.)
#   )
# }
...
ggplot(raw) + aes(reorder(ID, Ch2.Mean.), Ch2.Mean.) + geom_violin() +rx + facet_wrap(~Z.Level.) +
  ggtitle("H2B intensity") + ylim(0,10000)
