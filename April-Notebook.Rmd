...
# Now suppose I scaled the previous within-ROI
```{r, fig.height = 10, fig.width=10}

# z-scored DF
df.zroi = ddply(kk.long, .(CHANNEL, SUBROI), transform, z = scale(DENSITY))

# plot
ggplot(df.zroi) + aes(SUBROI, z) + 
  stat_summary(fun.y = mean, geom="point") + 
  stat_summary(fun.data = mean_se, geom="linerange") + 
  geom_hline(yintercept=0) + 
  theme(axis.text.x = element_text(angle = 90, hjust=1)) +
  facet_wrap(VALENCE~CHANNEL ,ncol = 2)
...
library(plyr)
ol.induction = ddply(kk.long, .(CHANNEL, BATCH), transform, norm = scale(DENSITY))
