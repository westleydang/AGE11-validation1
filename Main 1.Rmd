    ...
    for (counts in ch) {
        ...
        print(
            ggplot(
                ...
                labs(title = title) +rx + facet_wrap(~raw$EXPT)
            )
    }
}

heat_set(channels_mean, raw$GROUP, raw$System.)
heat_set(channels_hiSD, raw$GROUP, raw$System.)
heat_set(channels_lowSD, raw$GROUP, raw$System.)
heat_set(channels_midSD, raw$GROUP, raw$System.)
heat_set(channels_total, raw$GROUP, raw$System.)
...
ggplot(raw, aes(GROUP, OL.Total.)) + geom_col() + facet_wrap(~raw$EXPT)
ggplot(raw, aes(GROUP, Ch1.Total.)) + geom_col() + facet_wrap(~raw$EXPT)
ggplot(raw, aes(GROUP, Ch2.Total.)) + geom_col() + facet_wrap(~raw$EXPT)
ggplot(raw, aes(GROUP, Ch3.Total.)) + geom_col() + facet_wrap(~raw$EXPT)
ggplot(raw, aes(GROUP, Ch4.Total.)) + geom_col() + facet_wrap(~raw$EXPT)
