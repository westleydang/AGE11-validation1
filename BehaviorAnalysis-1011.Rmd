...
learning.aov = with(shock_only, aov(Pct.Component.Time.Freezing ~ EXPT * Component.Name + 
                                      Error(ID / (EXPT * Component.Name))))
learning.aov
summary(learning.aov)
...
shock_only %>%  aged() %>%
  ...
  scale_color_brewer(palette="Set1") + remove_grids() +
  scale_y_continuous(expand = c(0, 0))
...
rc.aov = rc_avg %>%
  do(tidy(aov(data=., avg.rc ~ EXPT * VALENCE)))

rc.tukey = rc_avg %>% 
  do(tidy(TukeyHSD(aov(data=., avg.rc ~ EXPT * VALENCE))))
