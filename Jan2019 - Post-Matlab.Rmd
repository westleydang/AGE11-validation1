...
compare.relevant = function(df) {
  ...
  keeps = sapply(mapply(match, sideA.elements, sideB.elements),
  function(x)
  length(x) - length(na.omit(x)))
  keeps = sapply(keeps, function(x)
  x == 1)
...
tot.lr = total.weighted %>%
  ...
  # filter(!CHANNEL %in% c('DAPI','GFAP')) %>%
  group_by(CHANNEL, EXPT) %>%
  do(glance(lm(data=., formula = FREEZE ~ weighted))) %>%
  put_sig_scores() 
  # 
# SPIT
tot.lr
...
sys.weighted %>%
  ...
  ggtitle("systems, weighted, density total cells") + 
  theme_light() + rx
...
# systems, weighted by ROI area VALENCE
sys.weighted %>%
  filter(CHANNEL %in% channel.list[c(1)]) %>%
  ggplot() + aes(VALENCE, weighted, color=EXPT) + 
  stat_summary(fun.y = "mean", geom="point") + 
  stat_summary(fun.data = "mean_se", geom="errorbar", aes(width=0.2)) + 
  facet_grid(CHANNEL~System, scales="free") +
...
sys.weighted %>%
  ...
  stat_summary(fun.y = "mean", geom="point") + 
  stat_summary(fun.data = "mean_se", geom="errorbar", aes(width=0.2)) + 
  facet_wrap(~System, scales="free", nrow=3) +
  ggtitle("GFAP, systems, weighted density, total cells") + 
  theme_light() + rx +
...
sys.weighted %>%
  ...
  stat_summary(fun.data = "mean_se", geom="errorbar", aes(width=0.2)) + 
  facet_wrap(~System, scales="free", nrow=3) +
  ggtitle("ArcIHC, systems, weighted density, total cells") + 
  theme_light() + rx +
  theme(legend.position="bottom")
```
## GFP PLOTS Full
...
sys.weighted %>%
  ...
  stat_summary(fun.data = "mean_se", geom="errorbar", aes(width=0.2)) + 
  facet_wrap(~System, scales="free", nrow=3) +
  ggtitle("H2BGFP, systems, weighted density, total cells") + 
  theme_light() + rx +
  theme(legend.position="bottom")
```
...
# systems, weighted by ROI area GROUP
sys.weighted %>%
  filter(CHANNEL %in% channel.list[c(6)]) %>%
  ggplot() + aes(GROUP, weighted, color=EXPT) + 
  stat_summary(fun.y = "mean", geom="point") + 
  stat_summary(fun.data = "mean_se", geom="errorbar", aes(width=0.2)) + 
  facet_wrap(~System, scales="free", nrow=3) +
  ggtitle("Normalized Overlap, systems, weighted density, total cells") + 
  theme_light() + rx +
  theme(legend.position="bottom")
```
...
sys.weighted %>%
  ...
  filter(EXPT=='OLD') %>%
  filter(VALENCE == 'SHOCK') %>%
  dcast(ID + CONTEXT + System~ CHANNEL, value.var="weighted") %>%
  group_by(System) %>%
  # do(tidy(lm(., formula = ArcIHC ~ GFAP))) %>% put_sig_scores()
  ggplot() + aes(GFAP, ArcIHC) + geom_point(aes(color=CONTEXT)) + facet_wrap(~System,ncol=5, scales="free") + geom_smooth(method="lm") + 
  theme_minimal()


# GRAPH FREEZE VS GFAP
sys.weighted %>%
  group_by(CHANNEL, System) %>%
  # mutate(weighted.z = scale(weighted)) %>%
  filter(EXPT=='OLD') %>%
  ...
  group_by(System) %>%
  # do(tidy(lm(., formula = ArcIHC ~ GFAP))) %>% put_sig_scores()
  ggplot() + aes(GFAP, FREEZE) + geom_point(aes(color=CONTEXT)) + facet_wrap(~System,ncol=5, scales="free") + geom_smooth(method="lm") + 
  theme_minimal()
...
sys.weighted %>%
  ...
  filter(EXPT=='OLD') %>%
  filter(VALENCE == 'SHOCK') %>%
  dcast(ID + System ~ CHANNEL, value.var="weighted") %>%
  merge(., animal_details %>% dplyr::select(FREEZE, ID), by="ID") %>%
  mutate(GFAP = round(GFAP,0)) %>%
  group_by(System) %>%
  do(tidy(lm(., formula = GFAP ~ FREEZE))) %>% 
...
for(ch in channel.list[c(2,3,6)]) {
      print(
        sys.weighted %>%
          ...
          facet_wrap(~System, scales="free", ncol=5) + geom_smooth(method="lm") +
          theme_light() +
          ggtitle(paste("Systems cell density vs freezing", ch)) + 
          xlab("System") + ylab("Normalized Overlap")
   )
}
...
sys.lr = sys.weighted %>%
  ...
  group_by(CHANNEL, System) %>%
  do(tidy(lm(data=., weighted ~ FREEZE))) %>% 
  filter(!term=="(Intercept)") %>%
  # mutate(p.value = p.adjust(p.value, method="BH", n=15)) %>%
  put_sig_scores()
sys.lr
...
# WHICH ARE SIG?
sys.lr.expt %>% filter(p.value < 0.05)
...
# SYSTEMS SPLIT AGE GROUPS
```{r}
# Do ANOVA for just young and aged separately?

ss3 = sys.weighted %>%
  filter(EXPT=='YOUNG') %>%
  filter(VALENCE=='SHOCK') %>%
  group_by(CHANNEL, System) %>%
  do(tidy(aov(data=., weighted ~ CONTEXT))) %>%
  mutate(p.value = p.adjust(p.value, method="BH", n=15))
summary(ss3$p.value)

# how many significant systems?
ss3.sig = ss3 %>% filter(p.value < 0.05) 
ss3.sig
summary(ss3.sig)

# make graphs
for (tm in unique(ss3$term)) {
    print(
      ss3 %>% 
        filter(term==tm) %>% 
        filter(!CHANNEL=='DAPI') %>%
...
```{r}
total.weighted %>% filter(CHANNEL==c('H2BGFP', 'ArcIHC')) %>% 
...
total.weighted %>%
  ddply(., .(ID, GROUP, EXPT, VALENCE, CONTEXT, CHANNEL), summarize, weighted = mean(weighted)) %>%
  dcast(., ID + GROUP + EXPT + VALENCE + CONTEXT ~ CHANNEL, value.var="weighted") %>%
  mutate(ArcIHC = 100*ArcIHC/DAPI,
         GFAP = 100*GFAP/DAPI,
         H2BGFP = 100*H2BGFP/DAPI,
         NormOL = 10*NormOL/DAPI
         ) %>%
  melt(., id.vars=c("ID", "GROUP", "EXPT", "VALENCE", "CONTEXT"), 
       variable.name="CHANNEL", value.name="pct") %>%
  filter(CHANNEL==c("ArcIHC", "H2BGFP", "NormOL")) %>%
  ggplot() + aes(CHANNEL, pct) + 
  stat_summary(fun.y = "mean", geom="bar", position="dodge") + 
  stat_summary(fun.data = "mean_se", geom="errorbar", aes(width=0.3), position=position_dodge(1)) +
  ggtitle(paste(c("Whole brain, percent of DAPI"),"total","cells")) +
  theme_minimal() + rx +
  expand_limits(y=c(0,.35)) +ylab("Percent DAPI")
...
# SYSTEMS
sys.weighted %>%
  ddply(., .(ID, GROUP, EXPT, VALENCE, CONTEXT, CHANNEL, System), summarize, weighted = mean(weighted)) %>%
  dcast(., ID + GROUP + EXPT + VALENCE + CONTEXT + System ~ CHANNEL, value.var="weighted") %>%
  mutate(ArcIHC = 100*ArcIHC/DAPI,
         GFAP = 100*GFAP/DAPI,
         H2BGFP = 100*H2BGFP/DAPI,
         NormOL = 10*NormOL/DAPI,
         ) %>%
  melt(., id.vars=c("ID", "GROUP", "EXPT", "VALENCE", "CONTEXT", "System"), 
       variable.name="CHANNEL", value.name="pct") %>%
  filter(CHANNEL==c("ArcIHC", "H2BGFP", "NormOL")) %>%
  ggplot() + aes(CHANNEL, pct) + 
  stat_summary(fun.y = "mean", geom="bar", position="dodge") + 
...
## Relative labeling per system
```{r}
knew.molten3 %>% 
  ddply(., .(CHANNEL, System), summarize,
        area = mean(value)) %>%
  group_by(CHANNEL) %>%
  mutate(Percent.Area = 100*area/sum(area)) %>%
  ggplot() + aes(reorder(System, Percent.Area), Percent.Area, fill=System) +
  geom_bar(stat='identity') +
  coord_flip() + theme_minimal() + facet_wrap(~CHANNEL, scale="free") +
  ggtitle("Mean relative labeling in each system") +
  xlab("System") + ylab("Percent of all labeling") + rx
...
f1 = total.weighted %>% 
  ...
  theme_minimal() + 
  expand_limits(y=0) + 
  ylab("Weighted cell density") + xlab("Age groups") + scale_fill_brewer(palette = "GnBu")
...
f3 = knew.molten3 %>%
  ...
  ggplot() + aes(EXPT, weighted, fill=VALENCE) + 
  stat_summary(fun.y = "mean", geom="bar", position="dodge") + 
  stat_summary(fun.data = "mean_se", geom="errorbar", aes(width=0.2), position=position_dodge(1)) + 
  facet_wrap(~System, scales="free", ncol=8) +
  ggtitle("H2BGFP per system") + 
  theme_minimal()  + expand_limits(y=0)  +
...
total.weighted %>% 
  filter(CHANNEL %in% c('H2BGFP')) %>% aged() %>%
  ggplot() + aes(EXPT, weighted, fill=VALENCE) + 
  stat_summary(fun.y = "mean", geom="bar", position="dodge") + 
  stat_summary(fun.data = "mean_se", geom="errorbar", aes(width=0.3), position=position_dodge(1)) + 
  # facet_wrap(~CHANNEL, scales="free") +
  ...
  theme_minimal() + 
  expand_limits(y=0) + 
  ylab("H2BGFP (cells/mm^2)") + scale_fill_brewer(palette = "GnBu") +
  theme(legend.position = c(0.77,0.9), legend.title=element_blank(), 
        axis.title.x = element_blank(),axis.line=element_line()) + remove_grids() +
...
knew.molten3 %>% aged() %>%
  ...
  ggplot() + aes(FREEZE, weighted) + 
  stat_summary(fun.y="mean", geom="point") +
  facet_wrap(~EXPT, scales="free", ncol=1) + geom_smooth(method="lm") +
  theme_minimal() + xlab("Retrieval Freezing %") + ylab("H2BGFP (cells/mm^2)") +
  theme(legend.title=element_text(colour="white"),
        axis.line=element_line()) + remove_grids()
```
...
h1 = total.weighted %>% 
  ...
  theme_minimal() + 
  expand_limits(y=0) + 
  ylab("Weighted cell density") + xlab("Age groups") + scale_fill_brewer(palette = "PuRd")
...
h2 = knew.molten3 %>%
  ...
  stat_summary(fun.y="mean", geom="point") +
  facet_wrap(~EXPT, scales="free", ncol=3) + geom_smooth(method="lm") +
  ggtitle("ArcIHC vs. Recall Freezing", subtitle="Shock animals only") +
  theme_minimal() + xlab("Freeze at final recall") + ylab("Weighted cell density")
...
total.weighted %>% 
  filter(CHANNEL %in% c('ArcIHC')) %>% aged() %>%
  ggplot() + aes(EXPT, weighted, fill=VALENCE) + 
  stat_summary(fun.y = "mean", geom="bar", position="dodge") + 
  stat_summary(fun.data = "mean_se", geom="errorbar", aes(width=0.3), position=position_dodge(1)) + 
  theme_minimal() + 
  expand_limits(y=c(0,500)) + 
  ylab("ArcIHC (cells/mm^2)") + scale_fill_brewer(palette = "PuRd") +
  theme(legend.position = c(0.77,0.9), legend.title=element_blank(), 
        axis.title.x = element_blank(), axis.line = element_line()) + remove_grids() +
...
knew.molten3 %>% aged() %>%
  ...
  stat_summary(fun.y="mean", geom="point") +
  facet_wrap(~EXPT, scales="free", ncol=1) + geom_smooth(method="lm") +
  theme_minimal() + xlab("Retrieval Freezing %") + ylab("ArcIHC (cells/mm^2)") +
  theme(legend.title=element_text(colour="white"),
        axis.line=element_line()) + remove_grids() 
```
...
```{r, fig.width=5, fig.height=5}
n1 = total.weighted %>% 
  filter(CHANNEL %in% c('NormOL')) %>%
  ggplot() + aes(EXPT, weighted, fill=VALENCE) + 
  stat_summary(fun.y = "mean", geom="bar", position="dodge") + 
  stat_summary(fun.data = "mean_se", geom="errorbar", aes(width=0.3), position=position_dodge(1)) + 
  facet_wrap(~CHANNEL, scales="free") +
  ggtitle("Total brain NormOL density") +
  theme_minimal() + 
  expand_limits(y=0) + 
  ylab("Weighted cell density") + xlab("Age groups") + scale_fill_brewer(palette = "YlOrRd")
...
n2 = knew.molten3 %>%
  ...
  stat_summary(fun.y="mean", geom="point") +
  facet_wrap(~EXPT, scales="free", ncol=3) + geom_smooth(method="lm") +
  ggtitle("NormOL vs. Recall Freezing") +
  theme_minimal() + xlab("Freeze at final recall") + ylab("Weighted cell density")
...
n3 = knew.molten3 %>%
  ...
  ggtitle("NormOL per system") + 
  theme_minimal()  + expand_limits(y=0)  +
  theme(axis.text=element_text(size=7),
        strip.text.x = element_text(size = 7),
        legend.position="bottom") +
  ylab("NormOL (fraction)") + xlab("") + 
  scale_fill_brewer(palette = "YlOrRd")

as_ggplot(grid.arrange(n1, n2, n3, nrow=2, 
                       layout_matrix = rbind(c(1,2), c(3,3)),
                       heights=1:2)) + 
  draw_plot_label(c("A","B","C"), x=c(0,0.5,0), y=c(1,1,0.666))
...
### NOrm OL Fig small system
```{r, fig.width = 4, fig.height=2}
knew.molten3 %>%
  filter(CHANNEL=='NormOL') %>%
  filter(System %in% c("SUBIC-HPC", "PFC", "TEMP ASSOC CTX", "CORTICO-AMY")) %>%
  group_by(ID, System, VALENCE, EXPT, CHANNEL, variable) %>%
  summarize(weighted = weighted.mean(value, Pop_roiAreaMM2)) %>%
  ggplot() + aes(EXPT, weighted, fill=VALENCE) + 
  stat_summary(fun.y = "mean", geom="bar", position="dodge") + 
...
sys.weighted %>%
  ...
  stat_summary(fun.y = "mean", geom="point") + 
  stat_summary(fun.data = "mean_se", geom="errorbar", aes(width=0.2)) + 
  facet_wrap(EXPT~System, scales="free", ncol=4) +
  ggtitle("NormOL, systems, weighted density, total cells") + 
  theme_light() +
  theme(legend.position="left")
```


### NormOL 4 region t-tests
```{r}
ss4 = sys.weighted %>%
  filter(EXPT=='YOUNG')%>%
  mutate(GRAIN = paste(EXPT, VALENCE)) %>%
  group_by(CHANNEL, System) %>%
  do(tidy(aov(data=., weighted ~ GRAIN))) %>%
  mutate(p.value = p.adjust(p.value, method="BH", n=15))
ss4
summary(ss4$p.value)

nolsys = sys.weighted %>% filter(CHANNEL=='NormOL')
arcsys = sys.weighted %>% filter(CHANNEL=='ArcIHC')
gfpsys = sys.weighted %>% filter(CHANNEL=='H2BGFP')
...
test.areas = function(x, where) {
  for (system in nol.interesting.systems) {
      x = x %>% filter(System==system)
      aa.data = x %>% aa() %>% pull(weighted)
      ab.data = x %>% ab() %>% pull(weighted)
      print(system); print(t.test(aa.data, ab.data))  
  }
}
...
t.test(x %>% aa() %>% pull(weighted), x %>% ab() %>% pull(weighted))
...
## PFC
t1 = nolsys %>% old() %>% shk() %>% ab() %>% filter(System=='CORTICO-AMY') %>% pull(weighted)
t2 = nolsys %>% old() %>% ctx() %>% aa() %>% filter(System=='CORTICO-AMY') %>% pull(weighted)
t.test(t1, t2)
...
total.weighted %>% 
  filter(CHANNEL %in% c('NormOL')) %>% aged() %>%
  ggplot() + aes(EXPT, weighted, fill=VALENCE) + 
  stat_summary(fun.y = "mean", geom="bar", position="dodge") + 
  stat_summary(fun.data = "mean_se", geom="errorbar", aes(width=0.3), position=position_dodge(1)) + 
  # facet_wrap(~CHANNEL, scales="free") +
  ...
  theme_minimal() + 
  expand_limits(y=0) + 
  ylab("NormOL (cells/mm^2)") + scale_fill_brewer(palette = "YlOrRd") +
...
knew.molten3 %>% aged() %>%
  ...
  stat_summary(fun.y="mean", geom="point") +
  facet_wrap(~EXPT, scales="free", ncol=1) + geom_smooth(method="lm") +
  theme_minimal() + xlab("Retrieval Freezing %") + ylab("NormOL (cells/mm^2)") +
  theme(legend.title=element_text(colour="white"),
        axis.line = element_line()) + remove_grids() +
  scale_y_continuous(expand=c(0,0))
...
## GFAP FIGURES
```{r, fig.width=5, fig.height=5}
# set color palette for gfap
library(RColorBrewer)
my_reds = brewer.pal(n=9, "Reds")[c(3,8)]
...
j1 = total.weighted %>% 
  ...
  theme_minimal() + 
  expand_limits(y=0) + 
  ylab("Weighted cell density") + xlab("Age groups") + scale_fill_manual(values=my_reds)
...
sys.weighted %>%
  ...
  filter(CHANNEL %in% c('GFAP')) %>%
  filter(EXPT=='OLD') %>%
  lm(data=., formula = weighted ~ FREEZE) %>% summary()
sys.weighted %>%
  merge(., animal_details %>% dplyr::select(FREEZE,ID), by='ID') %>%
  filter(VALENCE == 'SHOCK') %>%
  filter(System=='SUBIC-HPC') %>%
...
sys.weighted %>%
  ...
  group_by(EXPT) %>%
  do(tidy(lm(data=., formula = weighted ~ FREEZE))) %>%
  mutate(p.value = p.adjust(p.value, method="bonferroni", n=2)) %>%
  filter(!term=="(Intercept)") %>%
  mutate(sig = 
           case_when(
             p.value < 0.001 ~ "***",
...
total.weighted %>%
  ...
  theme_minimal() + ylab("ArcIHC (z)") + xlab("GFAP (z)") + remove_grids()
...
sys.weighted %>%
  ...
  group_by(System) %>%
  # do(tidy(lm(., formula = ArcIHC ~ GFAP))) %>% put_sig_scores()
  ggplot() + aes(GFAP, ArcIHC) + geom_point() + facet_wrap(~System,nrow=1, scales="free") + geom_smooth(method="lm") + 
  theme_minimal() + ylab("ArcIHC (z)") + xlab("GFAP (z)") + remove_grids()
...
total.weighted %>%
  ...
  merge(., animal_details %>% dplyr::select(FREEZE, ID), by='ID') %>%
  dcast(ID + FREEZE~ CHANNEL, value.var="weighted.z") %>%
  lm(., formula = FREEZE ~ GFAP) %>% summary()

```


### PAPER figs GFAP
```{r, fig.width=1.00, fig.height=1.75}

total.weighted %>% 
  filter(CHANNEL %in% c('GFAP')) %>% aged() %>%
  ggplot() + aes(EXPT, weighted, fill=VALENCE) + 
  stat_summary(fun.y = "mean", geom="bar", position="dodge") + 
  stat_summary(fun.data = "mean_se", geom="errorbar", aes(width=0.3), position=position_dodge(1)) + 
  # facet_wrap(~CHANNEL, scales="free") +
  ...
  ylab("Weighted cell density (cells/mm^2)") + scale_fill_manual(values=my_reds) +
...
total.weighted %>% 
  filter(CHANNEL %in% c('GFAP')) %>% aged() %>%
  ggplot() + aes(EXPT, weighted, fill=EXPT) + 
  stat_summary(fun.y = "mean", geom="bar", position="dodge") + 
  stat_summary(fun.data = "mean_se", geom="errorbar", aes(width=0.3), position=position_dodge(1)) + 
  # facet_wrap(~CHANNEL, scales="free") +
  # ggtitle("Total brain H2BGFP density") +
...
sys.weighted %>%
  ...
  group_by(System) %>%
  # do(tidy(lm(., formula = ArcIHC ~ GFAP))) %>% put_sig_scores()
  ggplot() + aes(GFAP, FREEZE) + geom_point() + 
  facet_wrap(~System) +
  geom_smooth(method="lm") + 
  theme_minimal() + remove_grids() + xlim(c(300,1050)) +
  labs(x="GFAP (cells/mm^2)", y="Freezing %")
...
sys.weighted %>%
  filter(CHANNEL =='H2BGFP') %>%
  ggplot() + aes(System, weighted, color=EXPT) + 
  stat_summary(fun.y = "mean", geom="point") + 
  stat_summary(fun.data = "mean_se", geom="errorbar", aes(width=0.2)) + 
  facet_wrap(EXPT~VALENCE, nrow=1) +
  ggtitle("H2BGFP") + 
  theme_minimal() + rx + scale_fill_brewer()
...
sys.weighted %>%
  ...
  stat_summary(fun.y = "mean", geom="point") + 
  stat_summary(fun.data = "mean_se", geom="errorbar", aes(width=0.2)) + 
  facet_wrap(EXPT~VALENCE, nrow=1,scales="free") +
  ggtitle("ArcIHC") + 
  theme_light() + rx
