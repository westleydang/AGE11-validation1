...
knew.molten3 %>%
  ...
  dplyr::summarize(weighted = weighted.mean(value, Pop_roiAreaMM2)) %>%
  ggplot() + aes(CHANNEL, weighted) + 
  stat_summary(fun.y = "mean", geom="bar", position="dodge") + 
  stat_summary(fun.data = "mean_se", geom="errorbar", aes(width=0.3), position=position_dodge(1)) +
  facet_wrap(~SubROI_SubroiAGGR, ncol=12) +
  ggtitle(paste(c("Whole brain, weighted, density"),"total","cells, in hippocampus")) +
  theme_minimal() + rx +
  expand_limits(y=0)
...
## FREEZE Correlations (Shock ONLY)
...
knew.molten3 %>%
  ...
  facet_wrap(EXPT~CHANNEL, scales="free", ncol=4) + geom_smooth(method="lm") +
  ggtitle("Total brain density vs freezing, total cells, shock animals only") +
  theme_minimal()

# GET THE STATS (BOX-COX TRANSFORMATION)
tot.lr = total.weighted.box %>%
  merge(., animal_details %>% select(FREEZE, VALENCE, CONTEXT, ID), by='ID') %>%
  filter(VALENCE == 'SHOCK') %>%
...
a %>% filter(CHANNEL=='ArcIHC') %>%
  filter(System =='HYPOTHAL') %>%
  filter(str_detect(term, ':')) %>%
  pull(term) %>% unique() %>% as.vector() %>%
  str_split(':') %>% unlist()
a %>% filter(CHANNEL=='ArcIHC') %>%
  filter(System =='HYPOTHAL') %>%
  filter(!(term %in% NULL))
...
# WHY ISNT' THIS WORKING, CORTICO SHOULD STILL BE THERE 
a
```
...
#       ggtitle(paste("Systems cell density vs freezing, shock only", ch)) +
#       theme_minimal() 
#   )
# }
...
for(ch in channel.list[c(2,3,6)]) {
  print(
    sys.weighted %>%
      ...
      facet_wrap(EXPT~System, scales="free", ncol=15) + geom_smooth(method="lm") +
      theme_minimal() + ggtitle(ch) + xlab("System")
  )
}


 # GET THE STATS ON BOXED
sys.lr2 = sys.weighted.box %>%
  merge(., animal_details %>% select(FREEZE, ID), by='ID') %>%
  filter(VALENCE == 'SHOCK') %>%
  ...
  # filter(!term=="(Intercept)") %>%
  mutate(p.value = p.adjust(p.value, method="fdr", n=15)) %>%
  mutate(sig = 
           case_when(
             p.value < 0.0005 ~ "***",
             p.value < 0.005 ~ "**", 
