...
```
### PAPER figure A
```{r, fig.width=1.55, fig.height=1.75}

total.weighted.aged = total.weighted %>% aged()
...
total.weighted %>% 
  ...
  theme_minimal() + 
  expand_limits(y=0) + 
  ylab("Weighted cell density (cells/mm^2)") + xlab("Age groups") + scale_fill_brewer(palette = "GnBu") +
  theme(legend.position = "bottom") + remove_grids()
```
### PAPER figure B
```{r, fig.width=1, fig.height=1.75}
knew.molten3 %>% aged() %>%
  filter(VALENCE == 'SHOCK') %>%
  filter(CHANNEL=='H2BGFP') %>%
  group_by(ID, FREEZE,GROUP, EXPT, CHANNEL, variable) %>%
  ...
  theme_minimal() + xlab("Retrieval Freezing %") + ylab("Weighted cell density (cells/mm^2)") 
```
...
j1 = total.weighted %>% 
  ...
  theme_minimal() + 
  expand_limits(y=0) + 
  ylab("Weighted cell density") + xlab("Age groups") + scale_fill_brewer(palette = "Paired")
