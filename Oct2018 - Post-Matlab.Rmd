...
for (bright in c('low', 'mid', 'hi', 'total')) {
  print(
    knew.molten2 %>%
      ...
      theme_minimal() + 
      ggtitle("Weighted mean cell density for whole brain") + rx +
      expand_limits(y=0)
  )
}
```
...
## Non parametric 2 way anova for each age
```{r}

ts1 %>%
  filter(EXPT=='OLD') %>%
  group_by(CHANNEL) %>%
  do(tidy(friedman.test( wm ~ VALENCE | CONTEXT, data=.))) 
...
knew.molten2 %>%
  filter(CHANNEL %in% channel.list[c(2,6)]) %>%
...
for (tm in unique(reg.stats$term)) {
  for (ch in unique(reg.stats$CHANNEL)) {
    print(
      reg.stats %>% 
