...
arc = knew.molten3 %>% filter(CHANNEL=='ArcIHC') %>%
  mutate(perf = ifelse(FREEZE>25, 'hi', 'low'))
...
Is there a difference between context AA and context AB? That should be a control. 
```{r}
arccaa = arc %>%
  filter(GROUP=='CAA') %>%
  get_corrtest_sys()
...
compare_matrix_r(arccaa, arccab, "caa", "cab")
...
# WHAT IF I DID HI VS LO SHOCKERS
av1 = arc %>%
  filter(perf == 'low') %>%
  get_corrtest_sys()
...
cortest.normal(R1 = av1$r, R2 = av2$r, n1=max(av1$n), n2=max(av2$n))
...
cortest.normal(R1 = arc.age1$r, R2 = arc.age2$r, n1=max(av1$n), n2=max(av2$n))
