...
df3 = dcast(df2, GRAIN ~ GRAIN2, value.var = "z", fun.aggregate = mean)
...
heat.roi(df2, "ArcIHC", "OL")
heat.roi(df2, "DAPI", "OL")
...
for (each in channel.list) {
  print(
    df2 %>%
      filter(CHANNEL==each) %>%
      dcast(GRAIN ~ GRAIN2, value.var="z", fun.aggregate = mean) %>%
      select(-1) %>%
      correlate() %>%
      network_plot(color=c("red", "white", "white", "white", "dark blue"))
  )
}
