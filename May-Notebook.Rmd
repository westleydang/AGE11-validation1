...
# 2. Z-score densities within each (CHANNEL, SUBROI) across all animals

```{r scaled, echo=FALSE, fig.width=8, fig.height=8}
kl2 = kk.long
...
ggplot(kl2.scale) + aes(SUBROI, GRAIN, fill = z) + 
  ...
  ggtitle("Densities, z-scored per ROI")

```



## 2.1 But what if we want to cluster them? 



# 3. Density Correlation in each ROI, with hierarchical clustering
...
df3[,74:145] %>% correlate() %>% network_plot(colors=c("red", "red","white","white","blue"))
df3[,2:73] %>% correlate() %>% network_plot(colors=c("red", "red","white","white","blue"))
