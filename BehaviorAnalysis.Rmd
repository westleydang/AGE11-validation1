...
shock_only %>%
  ggplot() + aes(Component.Name, Pct.Component.Time.Freezing, color=EXPT, group=ID) +
  facet_wrap(~EXPT) + geom_point() + geom_line() + theme_minimal() +
  ggtitle("Individual freezing after each shock",
          subtitle="Shocked animals only") +
  xlab("Post-shock periods") + ylab("Percent Freezing")
...
```{r}
avg.psp = shock_only %>%
  filter(!(Component.Name == 'POST1' | Component.Name == 'POST2')) %>%
  ddply(.,
               .(ID), 
               summarize, 
...
avg.psp[avg.psp$avg < mean(avg.psp$avg) - 1.5*sd(avg.psp$avg),]
...
## How does pre-shock periods look in Aged vs Young animals?
No difference
```{r}
