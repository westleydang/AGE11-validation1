...
get_lda = function(df, dfname) {
  ...
  data.for.lda = merge(data.for.lda, animal_details %>% mutate(GRAIN = paste(EXPT, VALENCE)) %>% dplyr::select(ID, GRAIN), by="ID")
  ...
  p1 <- ggplot(dataset) + geom_point(aes(lda.LD1, lda.LD2, colour = GRAIN, shape = GRAIN), size = 2.5) + 
    ...
    theme(legend.position="top", legend.title=element_text(colour="white")) + 
